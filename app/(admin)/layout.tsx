...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-dvh flex-col">
      <header className="bg-pitch-700 text-chalk-050">
        <div className="mx-auto flex max-w-[860px] items-center gap-3 px-4 py-3">
          <Link href="/admin" className="min-w-0 flex-1">
            ...
            <span className="block truncate text-xs text-chalk-200">{identity.email}</span>
          </Link>

          {identity.role === 'super_admin' ? (
            <span className="shrink-0 rounded-full border border-floodlight px-2 py-0.5 text-xs font-semibold text-floodlight">
      ...
      <main id="main" className="mx-auto w-full max-w-[860px] flex-1 px-4 py-5 pb-24">
        {children}
      </main>
    </div>
  );
